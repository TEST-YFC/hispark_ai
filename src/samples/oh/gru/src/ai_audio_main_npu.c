...
/* Src Path */
#define TASKS_NPU_AI_INPUT_PATH "/user/sample_mfcc.bin"
#define TASKS_NPU_AI_MODEL_PATH "/user/gru.exeom"
...
#define TASKS_NPU_AI_MODEL_PATH "/etc/workspace/gru.om"
...
/* Model Param */
#define AI_NPU_AUDIO_SAMPLE_INPUT_MFCC_FRAME_NUM 25
enum {
    GRU_MODEL_INPUT_MFCC = 0,
    GRU_MODEL_INPUT_HIDDEN,
    AI_NPU_INPUT_NUM
};
enum {
    GRU_MODEL_OUTPUT_LOGITS = 0,
    GRU_MODEL_OUTPUT_HIDDEN,
    AI_NPU_OUTPUT_NUM
};

/* Input Buffer */
void *input_mfcc_data_buffer = NULL;
...
static OH_AI_Status ai_npu_sample_prepare_mfcc_data(struct ai_npu_param *sample_param)
{
    size_t input_mfcc_data_size = OH_AI_TensorGetDataSize(sample_param->inputs.handle_list[GRU_MODEL_INPUT_MFCC]);
    input_mfcc_data_buffer = malloc(input_mfcc_data_size * AI_NPU_AUDIO_SAMPLE_INPUT_MFCC_FRAME_NUM);
    if (input_mfcc_data_buffer == NULL) {
        ai_printf("[AI_NPU] malloc failed\n");
        return OH_AI_STATUS_FAILED;
    }
    ai_npu_sample_read_bin_to_buffer(TASKS_NPU_AI_INPUT_PATH, input_mfcc_data_buffer,
                                     input_mfcc_data_size * AI_NPU_AUDIO_SAMPLE_INPUT_MFCC_FRAME_NUM);
    return OH_AI_STATUS_SUCCESS;
}
...
static OH_AI_Status ai_npu_audio_sample_process(struct ai_npu_param *sample_param, int predict_times)
{
    ai_printf("[AI_NPU] ai_npu_audio_sample_process, the predict_times is %d \n", predict_times);

    /* Prepare Input Data */
    void *input_mfcc_data = OH_AI_TensorGetMutableData(sample_param->inputs.handle_list[GRU_MODEL_INPUT_MFCC]);
    void *input_hidden_states_data =
        OH_AI_TensorGetMutableData(sample_param->inputs.handle_list[GRU_MODEL_INPUT_HIDDEN]);
    if (input_mfcc_data == NULL || input_hidden_states_data == NULL) {
        ai_printf("[AI_NPU] OH_AI_TensorGetMutableData failed\n");
        return OH_AI_STATUS_FAILED;
    }
    size_t input_mfcc_data_size = OH_AI_TensorGetDataSize(sample_param->inputs.handle_list[GRU_MODEL_INPUT_MFCC]);
    size_t input_hidden_states_data_size =
        OH_AI_TensorGetDataSize(sample_param->inputs.handle_list[GRU_MODEL_INPUT_HIDDEN]);

    char *target_input_mfcc_data_buffer = (char *)input_mfcc_data_buffer + input_mfcc_data_size * predict_times;
    memcpy_s(input_mfcc_data, input_mfcc_data_size, (void *)target_input_mfcc_data_buffer, input_mfcc_data_size);
    if (predict_times == 0) {
        memset_s(input_hidden_states_data, input_hidden_states_data_size, 0, input_hidden_states_data_size);
    }
    ...
    /* PostProcess */
    void *output_hidden_states_data =
        OH_AI_TensorGetMutableData(sample_param->outputs.handle_list[GRU_MODEL_OUTPUT_HIDDEN]);
    if (output_hidden_states_data == NULL) {
        ai_printf("[AI_NPU] OH_AI_TensorGetMutableData failed\n");
        return OH_AI_STATUS_FAILED;
    }
    size_t output_hidden_states_data_size =
        OH_AI_TensorGetDataSize(sample_param->outputs.handle_list[GRU_MODEL_OUTPUT_HIDDEN]);
    int mem_ret = memcpy_s(input_hidden_states_data, input_hidden_states_data_size,
                           output_hidden_states_data, output_hidden_states_data_size);
    if (mem_ret != 0) {
        ai_printf("[AI_NPU] memcpy_s failed\n");
        return OH_AI_STATUS_FAILED;
    }
...
static void ai_npu_sample_destroy(struct ai_npu_param *sample_param)
{
    ...
    free(input_mfcc_data_buffer);
    input_mfcc_data_buffer = NULL;
}

static void ai_npu_task_audio_sample(void)
{
    ...
    ret = ai_npu_sample_prepare_mfcc_data(&sample_param);
    if (ret != OH_AI_STATUS_SUCCESS) {
        ai_printf("[AI_NPU] ai_npu_sample_prepare_mfcc_data failed (%d)\n", ret);
        return;
    }

    for (int predict_times = 0; predict_times < AI_NPU_AUDIO_SAMPLE_INPUT_MFCC_FRAME_NUM; ++predict_times) {
        ret = ai_npu_audio_sample_process(&sample_param, predict_times);
        if (ret != OH_AI_STATUS_SUCCESS) {
    ...
    ai_npu_sample_print_output_tensor(sample_param.outputs.handle_list[GRU_MODEL_OUTPUT_LOGITS]);
...
{
    unused(arg);
    ai_printf("[AI_NPU] excute audio sample\n");
    ai_npu_task_audio_sample();
    return NULL;
}
