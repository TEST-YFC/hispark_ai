...
${SDK_PATH}/middleware/utils/ai_mcu/lib"
    ...
    export LANG="C"
    # Training sample depends on newly added adaptor APIs. Always refresh the
    # adaptor headers/sources in SDK to avoid building against stale ai.h.
    mkdir -p "$SDK_PATH/middleware/utils/ai_mcu"
    rm -rf "$SDK_PATH/middleware/utils/ai_mcu/adaptor"
...
function build_cfbb()
{
    ...
    # Replace any stale AI custom sample block from previous builds. Otherwise
    # the SDK may keep compiling an old sample path and this task will not run.
    python - "$SDK_PATH/application/samples/CMakeLists.txt" "$CUR_DIR" <<'PY'
import re
import sys
...
content = re.sub(
    r"\nif\(DEFINED CONFIG_ENABLE_AI_CUSTOM_SAMPLE\)\n\s*add_subdirectory\(\n.*?\n\s*\)\nendif\(\)\n",
    "\n",
    content,
    flags=re.S,
)
block = (
    ...
    "  add_subdirectory(\n"
    f"    {sample_dir}\n"
    "    ${CMAKE_CURRENT_BINARY_DIR}/mnist_training_build\n"
    "  )\n"
    "endif()\n\n"
)
content = content.replace("add_subdirectory_if_exist(custom)", block + "add_subdirectory_if_exist(custom)")
...
PY
    ...
    cp $SDK_PATH/output/ws63/fwpkg/ws63-liteos-app/ws63-liteos-app_all.fwpkg \
        $CUR_DIR/output/ws63-ai-mnist-training-sample.fwpkg
}
