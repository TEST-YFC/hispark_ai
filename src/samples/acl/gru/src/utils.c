...
static const uint32_t g_inputSize[INPUT_NUM] = {20, 308};
static const uint32_t g_outputSize[OUTPUT_NUM] = {24, 308};
enum { GRU_MODEL_INPUT, GRU_MODEL_INPUT_HIDDEN };
enum { GRU_MODEL_OUTPUT, GRU_MODEL_OUTPUT_HIDDEN };
static bool g_memSwitch = false;
static void *g_modelInput = NULL;
static void *g_modelOutput = NULL;
static void *g_modelHidden = NULL;
...
{
    aclError ret;
    ret = aclrtMalloc(&g_modelInput, g_inputSize[GRU_MODEL_INPUT], ACL_MEM_MALLOC_NORMAL_ONLY);
    if (ret != ACL_SUCCESS) {
        osal_printk("aclrtMalloc failed, input_size[%u]\n", g_inputSize[GRU_MODEL_INPUT]);
        return FAILED;
    }
    ret = aclrtMalloc(&g_modelOutput, g_outputSize[GRU_MODEL_OUTPUT], ACL_MEM_MALLOC_NORMAL_ONLY);
    if (ret != ACL_SUCCESS) {
        osal_printk("aclrtMalloc failed, output_size[%u]\n", g_outputSize[GRU_MODEL_OUTPUT]);
        return FAILED;
    }
    uint32_t hiddenSize = g_inputSize[GRU_MODEL_INPUT_HIDDEN] + g_outputSize[GRU_MODEL_OUTPUT_HIDDEN];
    ret = aclrtMalloc(&g_modelHidden, hiddenSize, ACL_MEM_MALLOC_NORMAL_ONLY);
    if (ret != ACL_SUCCESS) {
        osal_printk("aclrtMalloc failed, hidden_size[%u]\n", hiddenSize);
        return FAILED;
    }
    ret = aclrtMemset(g_modelHidden, hiddenSize, 0, hiddenSize);
    if (ret != ACL_SUCCESS) {
        osal_printk("aclrtMemset failed\n");
        return FAILED;
    }
    return SUCCESS;
}
...
static void ModelMemFree(void)
{
    ...
    if (g_modelHidden != NULL) {
        aclrtFree(g_modelHidden);
        g_modelHidden = NULL;
    }
}
...
// 从文件中读取输入数据
static Result GetInputData(const char *filePath, uint32_t runTimes)
{
    uint32_t fileSize = g_inputSize[GRU_MODEL_INPUT];
    FILE *fp = NULL;
    ...
    (void)fseek(fp, runTimes * fileSize, SEEK_SET);
...
Result ModelExecute(ModelInfo *modelInfo, uint32_t runTimes)
{
    MemInfo *memInfo = &modelInfo->memInfo;
    void *inputAddr[INPUT_NUM];
    void *outputAddr[OUTPUT_NUM];
    // 从文件中读取输入数据
    if (GetInputData(modelInfo->inputFile, runTimes) != SUCCESS) {
