...
def create_exp_onnx_model(output_path):
    """创建 Exp 算子 ONNX 模型
    
    Args:
        output_path: 模型保存路径
    """
    logging.info(f"创建 Exp 模型: {output_path}")
    ...
    # 创建模型
    model = create_low_ir_version_model(graph, producer_name='exp-generator', output_path=output_path)

    logging.info(f"✓ Exp 模型已保存: {output_path}")