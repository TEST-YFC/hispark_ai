...
def create_add_onnx_model(output_path):
    """创建Add算子ONNX模型"""
    print(f"创建Add模型: {output_path}")
    ...
    # 创建模型
    model = create_low_ir_version_model(graph, producer_name='add-generator', output_path=output_path)

    print(f"✓ Add模型已保存: {output_path}")