...
def create_abs_onnx_model(output_path):
    """创建 Abs 算子 ONNX 模型
    
    Args:
        output_path: 模型保存路径
    """
    logging.info(f"创建 Abs 模型: {output_path}")
    ...
    # 创建模型
    model = create_low_ir_version_model(graph, producer_name='abs-generator', output_path=output_path)
    logging.info(f"✓ Abs 模型已保存: {output_path}")