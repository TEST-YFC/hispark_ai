...
def create_mathmodel_tflite_model(output_path):
    ...
    class MathOperatorsModel(tf.Module):
        ...
        def __call__(self, x):
            ...
            # 14. SplitV
            split_v_result = tf.split(x, [1, 2], axis=2, name="splitv")
            
            # 15. Concatenation
            x = tf.concat([split_result[0], split_v_result[0]], axis=1, name="concat")
            ...
            # 20. Resize Bilinear
            if len(x.shape) == 3:
                # 如果是3D，添加batch维度并转换为4D
                x_4d = tf.expand_dims(x, axis=0)
                x_resized = tf.image.resize(x_4d, [4, 6], method=tf.image.ResizeMethod.BILINEAR, name="resize_bilinear")
                x = tf.squeeze(x_resized, axis=0)
            
            # 21. Resize Nearest Neighbor
            if len(x.shape) == 3:
                x_4d = tf.expand_dims(x, axis=0)
                x_resized = tf.image.resize(x_4d, [4, 6], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR, name="resize_nearest")
                x = tf.squeeze(x_resized, axis=0)
