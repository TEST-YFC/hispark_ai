...
def create_reducemean_tflite_model(output_path):
    """创建 ReduceMean 算子的 TFLite 模型（适配MindSpore Lite转换）"""
    # 1. 定义 TensorFlow 模型（仅包含 ReduceMean 操作）
    axis_tensor = tf.constant(2, dtype=tf.int32, name="axis")
    
    class ReduceMeanModel(tf.Module):
        def __init__(self):
        ...
        def __call__(self, x):
            # 使用 tf.reduce_mean 进行ReduceMean计算
    ...
    logging.info(f"✓ ReduceMean TFLite 模型已保存: {output_path}")