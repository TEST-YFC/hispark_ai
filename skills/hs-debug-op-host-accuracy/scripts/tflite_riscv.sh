#!/bin/bash
# TFLite RISC-V driver (hs-debug-op-host-accuracy Step R2/R3). Same as ONNX riscv but --fmk=TFLITE + NHWC.
...
sed -i -E "s@^([[:space:]]*)element_num = element_num > (MAX_ELEMENT_NUM|10) \?.*@\1// print cap disabled by hs-debug-op-host-accuracy harness@" benchmark/benchmark.c 2>/dev/null
