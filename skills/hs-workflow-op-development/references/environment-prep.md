...
**优先级约定：** 默认 `AUTO_ALL` 必须先由用户在 Stage1 提供
`FIRMWARE_SDK_ROOT` 绝对路径；没有路径时停在 Stage1，不自动下载或从磁盘挑选 SDK。
...
| 没有 SDK，且明确要求编译/上板 | 停在 Stage1，要求用户提供 SDK 绝对路径；需要下载时先单独完成环境准备请求，再以实际路径开始新 run | 不在本算子 run 内调用 |
...
该门禁针对每一位使用者自己的 Codex/Skill 环境在 Stage1 执行。通知发生在该使用者
...
3. 若固件阶段需要补环境，按名称加载用户提供或已安装的 `hs-dev-env-prep`，
   ...
   外部 Skill；将安装路径和恢复命令写入状态。用户安装或提供该 Skill 后，从 Stage1 重新检查；
   不需要重做已通过的 Host 阶段。
...
默认`BOARD_POLICY=AUTO_ALL`在Stage1必须检查用户本次请求或当前会话是否已经明确提供
`FIRMWARE_SDK_ROOT=<固件SDK仓库绝对路径>`。没有时必须向用户询问并停在Stage1，不能通过
`EXECUTION_CONFIRM_GATE`或进入Stage2；禁止通过搜索磁盘、其他任务记录、环境变量或fbb自动
选择一个可写SDK。只有用户明确切换为`BOARD_POLICY=HOST_ONLY`时，才可不提供SDK并继续
Stage2-Stage5，同时将 Stage6/Stage7 记为`NOT_REQUESTED`。用户提供后，记录对应 `FIRMWARE_SDK_SRC`，再检查
