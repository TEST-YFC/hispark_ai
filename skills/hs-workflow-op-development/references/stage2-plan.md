...
> 本文件对应 Stage2，任务 ID 使用 `stage2.*`。

## 目录

- [固定顺序](#stage2实现计划和初版文档先行)
- [源码冻结和回流](#源码冻结和回流)

> 进入 Stage2 后按入口表中的顺序读取。这里保留 prepare、integrated-initial、pre-source 的完整门禁关系。

## Stage2：实现计划和初版文档先行

仅当`EXECUTION_CONFIRM_GATE=PASS`时进入本阶段；`PENDING`或缺少记录都必须停在Stage1。

进入本阶段先将 `stage2.plan` 标为 `RUNNING`，并在计划、初版文档和
...
发生变化，都使本轮Stage2失败，必须查明来源并重新执行prepare，不能用后补文档掩盖顺序错误。

同一`OP_PLAN_RUN_ID`禁止覆盖freeze receipt。上一轮Stage2已结构化终止并明确开始新规划轮次时，
才可生成新ID并显式rotate；旧receipt必须归档，不得静默重置基线。固件SDK不属于本门禁范围，
其写入授权、接线receipt和新鲜度由Stage6单独门控。

实现过程中若规格、实现约定、能力清单或计划用例需要变化，停止源码修改并回到Stage2完整重跑
