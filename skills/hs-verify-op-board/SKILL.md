...
description: >-
  ...
  generate and cross-compile each Micro model, generate the OH_AI sample, verify wiring, hand builds
  and flash back to the top-level workflow, collect complete serial tensors, and report per-case plus
...
| `verify-prepared-firmware` | step4 | 用户明确已有身份可追踪的构建/烧录产物，只恢复证据并比较 |
...
才按异常规则请求动作，并同时把当前行与剩余行状态落盘；不能把请求动作误当作重新确认整个流程。
...
1. [`references/workflow-gates.md`](references/workflow-gates.md)：职责边界、step0/0a、矩阵冻结和进度模板。
...
5. [`references/board-guardrails.md`](references/board-guardrails.md)：红线、完成判据、失败措辞和资源所有权。
...
至少包含 `run_id/operator/framework/case_id/mode/status/reason` 以及同轮 model、input、GT、firmware、
...
板端未执行时，板端结果为 `ACCURACY_VERDICT=NOT_RUN`，整体由 workflow 结案为
...
| [`references/board-guardrails.md`](references/board-guardrails.md) | 红线、完成判据和分流 |
...
| [`chips/ws63/references/troubleshooting.md`](chips/ws63/references/troubleshooting.md) | WS63 接线/构建失败回流 |
