#!/usr/bin/env python3
# coding: utf-8
"""hs-verify-op-host FIXED harness — operator-agnostic. Run IN PLACE from the skill's scripts/.
...
-----
    ...
    export MSLITE_PKG=<absolute unpacked package path>
...
def load_spec(spec_path: Path, frameworks=None):
    ...
    if not hasattr(mod, "PARAM_COLUMNS"):
        mod.PARAM_COLUMNS = []
    if mod.ONNX_TEST_CASES and not getattr(mod, "ONNX_TARGET_OP_TYPE", None):
        sys.exit("[ERROR] ONNX_TEST_CASES 非空但未声明 ONNX_TARGET_OP_TYPE；"
                 "无法防止模型 API 把目标节点静默改写成别的算子。")
...
def parse_benchmark_outputs(stdout):
    """Compatibility view used by parser tests and callers that only need values."""
    return [tensor["data"] for tensor in parse_benchmark_tensors(stdout)]
...
def judge_path_from_stdout(case_dir, path_key, stdout, stderr="", rc=0, spec=None, build_dir=None):
    ...
    bench_tensors = parse_benchmark_tensors(stdout)
    if not bench_tensors:
    ...
    for i, (tensor, ref) in enumerate(zip(bench_tensors, refs)):
        device = tensor["data"]
        if tensor["shape"] is None:
            msg = f"output tensor shape metadata missing for output[{i}]"
            _write_judge_report(log_dir, path_key, "FAIL", None, msg)
            return "FAIL", None, msg
        if tensor["elements"] is not None and tensor["elements"] != device.size:
            msg = (
                f"output tensor data truncated output[{i}] "
                f"declared={tensor['elements']} vs parsed={device.size}"
            )
            _write_judge_report(log_dir, path_key, "FAIL", None, msg)
            return "FAIL", None, msg
        if device.size != ref.size:
            msg = (
                f"output tensor element count mismatch output[{i}] "
                f"gt={ref.size} vs device={device.size}"
            )
            _write_judge_report(log_dir, path_key, "FAIL", None, msg)
            return "FAIL", None, msg
        if tensor["shape"] != ref.shape:
            msg = (
                f"output tensor shape mismatch output[{i}] "
                f"gt={ref.shape} vs device={tensor['shape']}"
            )
            _write_judge_report(log_dir, path_key, "FAIL", None, msg)
            return "FAIL", None, msg
...
def _write_rerunnable_driver(log_dir, build_dir, raw_driver, path_key, case_dir, spec_path=None):
    ...
    wrapper = f"""#!/bin/bash
# Rerunnable hs-verify-op-host path wrapper. Safe to run from any CWD.
set -u
...
def _converter_encryption_capability(mslite_pkg):
    """Return (argument, diagnostic) for the exact converter in this package.

    Converter CLI options differ between MindSpore Lite releases.  In particular,
    2.8 rejects ``--encryption=false`` at startup.  Probe the package being tested
    instead of assuming a version, and cache by resolved converter path so a matrix
    run does not execute ``--help`` once per case.
    """
...
def run_driver(framework, path_key, build_dir, log_dir, mslite_pkg, model_file, cfg_file, input_files,
    ...
    content = f"echo {shlex.quote(capability_log)}\n" + content
...
def _onnx_op_types(model_path):
    """Exact op_type values present in the generated ONNX graph."""
    import onnx
    model = onnx.load(str(model_path))
    return [node.op_type for node in model.graph.node]


def _assert_target_source_op(spec, framework, model_file):
    """Reject a generated source model that no longer contains the operator under test."""
    if framework == "onnx":
        target = getattr(spec, "ONNX_TARGET_OP_TYPE", None)
        actual = _onnx_op_types(model_file)
        if target not in actual:
            raise RuntimeError(
                f"OP_MISMATCH: ONNX model op_types={actual}, target op_type {target!r} absent — "
                "the model-building API rewrote/folded the target node into another operator. "
                "Build the target node explicitly or redesign the case; do not test the substitute.")
        return
...
def assert_int8_genuine(spec, work_dir):
    ...
    if the op is missing from support_int8_ops_ (implement ⑦) or its int8 OpCoder isn't
...
def _err_msg(stdout, stderr, work_dir, rc=0):
    ...
    fix is in the operator's quant path (hs-dev-op-implement side), not here.
    """
    blob = stdout + "\n" + stderr
    low = blob.lower()
...
def run_framework(framework, active_paths, mslite_pkg, spec, project_dir, verbose):
    ...
    for tc in cases:
        ...
        try:
            ...
            _assert_target_source_op(spec, framework, model_file)
...
# --------------------------------------------------------------------------- #
# Capability-coverage gate: the implement step3 checklist, frozen as JSON, must stay covered
# --------------------------------------------------------------------------- #

def load_capability_checklist(project_dir: Path):
    """hs-dev-op-implement step3 的「能力验收清单」frozen as JSON at
...
def validate_checklist_refs(checklist, spec, frameworks):
    ...
    if problems:
        sys.exit("[ERROR] capability_checklist.json 对账失败（开跑前拦截，防能力清单被悄悄改写）:\n"
                 ...
                 "        对账方向单向——以 implement step3 能力清单为准改 spec，不得反向删/改能力行来匹配存量 spec。")
...
def main():
    ...
    threshold_overridden = (
        ...
        or args.threshold_int8 != DEFAULT_THRESHOLD_INT8
    )
    if threshold_overridden and not args.judge_case:
        sys.exit(
            "[ERROR] 完整 Host 签收只允许固定阈值 "
            f"fp32={DEFAULT_THRESHOLD_FP32}, int8={DEFAULT_THRESHOLD_INT8}；"
            "自定义 --threshold-* 仅可与 --judge-case 一起用于非签收诊断。"
        )
    ...
    if args.judge_case:
        if threshold_overridden:
            print("VERDICT_NON_SIGNABLE=1 reason=custom_thresholds_judge_only")
    ...
    if checklist:
        ...
        print("[warn] 未发现 scripts/capability_checklist.json —— 能力覆盖无法机械校验。"
              "hs-dev-op-implement step3 应落盘该清单；缺失时 VERDICT 不含能力覆盖留痕。")
