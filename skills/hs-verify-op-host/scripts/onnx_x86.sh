#!/bin/bash
# ONNX x86 fp32 driver (hs-verify-op-host internal step2/step3/step5). Filled by run_all_cases.py and run via `bash`.
# NOT sourced -> plain `exit 1` is safe; we deliberately avoid `set -e`.
...
sed -i -E "s@^([[:space:]]*)element_num = element_num > (MAX_ELEMENT_NUM|10) \?.*@\1// print cap lifted by hs-verify-op-host harness (full-tensor dump for Python-side cosine)@" benchmark/benchmark.c 2>/dev/null
