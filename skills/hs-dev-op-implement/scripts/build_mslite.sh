...
#
# build_mslite.sh [--full] [构建根目录]      启动一次构建（带并发锁）
# build_mslite.sh --wait [max_secs]         阻塞等待构建结束（内部轮询；推荐，免 sleep 算术）
# build_mslite.sh --status                  报告当前/上次构建状态（即时返回）
# build_mslite.sh --stop                    终止当前构建（杀整个进程组，含 make/编译器子进程）
#
# hs-dev-op-implement 标准编译入口，一条命令完成：
...
#
# 启动（后台运行，编译 10–30 分钟；日志由脚本自写 /tmp/mslite_build.log，调用方无需也不要自行重定向到别的文件——
# --status 只认固定路径，换了文件名它会 tail 到上一轮的陈旧内容）：
...
#   bash <skill>/scripts/build_mslite.sh --wait 540
...
#   bash <skill>/scripts/build_mslite.sh --status
...
PIDFILE=/tmp/mslite_build.pid
RCFILE=/tmp/mslite_build.rc
LOGFILE=/tmp/mslite_build.log
PGIDFILE=/tmp/mslite_build.pgid   # setsid 后的构建进程组组长，--stop 据此整组清场
STARTFILE=/tmp/mslite_build.start # 本轮启动时间戳，--status 据此显示已运行时长
...
if [ "${1:-}" = "--status" ]; then
  if [ -f "${PIDFILE}" ] && kill -0 "$(cat "${PIDFILE}")" 2>/dev/null; then
    el=""
    if [ -f "${STARTFILE}" ]; then
      _now=$(date +%s); _start=$(cat "${STARTFILE}")
      el="，已运行 $(( (_now - _start) / 60 ))m$(( (_now - _start) % 60 ))s"
    fi
    echo "RUNNING (PID=$(cat "${PIDFILE}")${el})——本轮日志尾部："
  ...
  # 修复动作只能基于下面的符号名与失败 target；没拿到符号名前禁止清缓存/全量重建（见 troubleshooting.md 链接条目）
...
if [ "${1:-}" = "--wait" ]; then
  ...
  exec "$0" --status   # 构建已结束（或从未启动）——复用 --status 的成败摘要与退出码
fi
...
trap 'rc=$?; echo ${rc} > "${RCFILE}"; rm -f "${PIDFILE}" "${PGIDFILE}"' EXIT
# 自写固定日志（先截断旧内容）：--status 只认 ${LOGFILE}，与调用方是否/如何重定向无关，
# 杜绝"新一轮 RUNNING 却 tail 出上一轮 BUILD OK"的陈旧假信号
exec > "${LOGFILE}" 2>&1
echo "[build_mslite] 本轮启动: $(date '+%F %T')  日志: ${LOGFILE}"
...
if [ ${rc} -ne 0 ]; then
  echo "[!] 编译失败（exit ${rc}）——跑 --status 看错误摘要，或在日志中检索 'error:'" >&2
  exit 3
fi
