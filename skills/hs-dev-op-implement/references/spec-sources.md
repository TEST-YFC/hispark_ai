...
带着这些问题整理笔记（不抄代码，只记理解）：算法公式、属性如何影响计算、输入输出形状关系、**边界情况（空输入/单元素/全零/标量）**、数据类型约束、中间量精度（float vs double）。对索引类输入还必须记录索引域、负索引是否允许以及越界时的规范行为；不能由实现者凭经验把负索引当成不支持，或静默截断/取模。
